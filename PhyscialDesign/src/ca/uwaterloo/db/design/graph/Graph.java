...
public class Graph {
	 ...
	 */
	public GraphPath getMinCostPath(SimplePath p, Graph graph) throws NoPathFoundException {
	...
	/**
	 * Use DFS to find the minimal cost match for path started at pNode. 
	 * The current node of graph is the stating node in the graph to be matched.
	 * 
	 * @param depth records the level of recursion, which is used as cost for now
	 * @param path records the current path from the 'current' to the latest 
	 * matched graph node.
	 */
	private void dfsMatch(GraphNode gNode, PathNode pNode, int depth, List<GraphNode> path) {
